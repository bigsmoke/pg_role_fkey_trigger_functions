...
do $$
declare
    _tables_to_trust regclass[];
    _trusting_role name;
begin
    for _trusting_role, _tables_to_trust in
        select
            case
                when pg_roles.rolsuper then null
                else pg_roles.rolname
            end
            ,array_agg(pg_trigger.tgrelid)::regclass[]
        from
            pg_catalog.pg_proc
        inner join
        ...
        where
            ...
            and pg_namespace.nspname not like 'pg_temp%'
        group by
            pg_roles.rolname
            ,pg_roles.rolsuper
    loop
        raise warning using
            message = format(
                'Marking the following tables as trusted %s in database `%I`: `%L`'
                ,case
                    when _trusting_role is null then
                        'globally'
                    else
                        format('by `%I`', _trusting_role)
                end
                ,current_database()
                ,_tables_to_trust::text[]::text
            )
            ,hint = format(
                'Note that this will require either `SUPERUSER` or `CREATEROLE` privilege for the role'
                ' doing the extension update—most likely `SUPERUSER`.  Sorry about that little betrayal'
                ' against the `superuser = false` in the `.control` file.  Reinstall instead of update'
                ' this extension if that is a no-go for your situation.'
            )
        ;
        perform select pg_role_fkey_trigger_functions__trust_tables(_tables_to_trust, _trusting_role);
    end loop;
end;
$$;
